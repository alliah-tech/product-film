...
| Format | `▭ Format` on the start screen, in the pause menu and in the controls: **Landscape 16:9** (1920×1080) or **Vertical 9:16 (phone)** (1080×1920); `?format=portrait`; remembered per film. The stage changes size and `#app` gets `fmt-portrait`: the film restyles its scenes under that class (phone bigger and centered, captions on top, brand at the bottom). Selector-anchored cursor waypoints follow the layout by themselves; coordinate ones go through `FILM_FORMATS.portrait.mapXY(x, y)` when the film defines it (declare `var FILM_FORMATS = {...}` in the tables). A take records at the stage size — a vertical take plays upright on WhatsApp. Ship the vertical layout whenever the audience watches on a phone |
