...
test('local file: decodes, selects and records with audio', async ({ page }) => {
  ...
  await page.setInputFiles('#mp-file', { name: 'theme.wav', mimeType: 'audio/wav', buffer: wavBuffer(1, 440) });
  await page.waitForFunction(() => window.__film.studio.music.sel.type === 'file', null, { timeout: 10000 });
  await expect(page.locator('#st-music-name')).toHaveText('theme.wav');
